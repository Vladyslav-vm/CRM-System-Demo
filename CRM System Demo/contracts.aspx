...
<body>
    ...
    <asp:Button ID="main" runat="server" Text="Main" OnClick="main_Click" />
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:CRMBaseConnectionString %>" DeleteCommand="DELETE FROM [_contract] WHERE [id] = @original_id AND [date_conclusion] = @original_date_conclusion AND [total_price] = @original_total_price AND [contract_balance] = @original_contract_balance AND [date_end] = @original_date_end AND [id_customer] = @original_id_customer AND [contract_type] = @original_contract_type AND [id_product] = @original_id_product" InsertCommand="INSERT INTO [_contract] ([date_conclusion], [total_price], [contract_balance], [date_end], [id_customer], [contract_type], [id_product]) VALUES (@date_conclusion, @total_price, @contract_balance, @date_end, @id_customer, @contract_type, @id_product)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [_contract] ORDER BY [id]" UpdateCommand="UPDATE [_contract] SET [date_conclusion] = @date_conclusion, [total_price] = @total_price, [contract_balance] = @contract_balance, [date_end] = @date_end, [id_customer] = @id_customer, [contract_type] = @contract_type, [id_product] = @id_product WHERE [id] = @original_id AND [date_conclusion] = @original_date_conclusion AND [total_price] = @original_total_price AND [contract_balance] = @original_contract_balance AND [date_end] = @original_date_end AND [id_customer] = @original_id_customer AND [contract_type] = @original_contract_type AND [id_product] = @original_id_product">
            <DeleteParameters>
                <asp:Parameter Name="original_id" Type="Int32" />
                <asp:Parameter DbType="Date" Name="original_date_conclusion" />
                <asp:Parameter Name="original_total_price" Type="Decimal" />
                <asp:Parameter Name="original_contract_balance" Type="Decimal" />
                <asp:Parameter DbType="Date" Name="original_date_end" />
                <asp:Parameter Name="original_id_customer" Type="Int32" />
                <asp:Parameter Name="original_contract_type" Type="String" />
